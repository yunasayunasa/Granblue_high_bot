...
client.on('interactionCreate', async interaction => {
  try {
    // デバッグログ
    console.log(`インタラクション検出: ${interaction.type}, カスタムID: ${interaction.customId || 'なし'}`);
    ...
    if (interaction.isButton()) {
      ...
      // 日付選択
        ...
        const [_, __, raidType, dateStr] = customId.split('_');
        await showTimeSelection(interaction, raidType, dateStr);
      }
      // 募集確定ボタン
      else if (customId.startsWith('confirm_recruitment_')) {
      ...
      // 参加確定ボタン（timeflowテストと同じパターンに修正）
      else if (customId.startsWith('confirm_')) {
        console.log('参加確定ボタンが押されました');
        
        try {
          // 募集IDを抽出
          const recruitmentId = customId.split('_')[1];
          console.log(`参加確定 - 募集ID: ${recruitmentId}`);
          ...
          // タイムスタンプ用の一時データを取得（この情報は別途保存する必要があります）
          // 実装方法の例: 別のMapオブジェクトに時間情報を保存
          const userSelections = tempUserSelections.get(interaction.user.id) || {};
          const joinType = userSelections.joinType || 'なんでも可';
          const selectedAttributes = userSelections.attributes || ['火', '水', '土', '風', '光', '闇'];
          const timeAvailability = userSelections.timeAvailability || 'now';
          ...
          // 一時データを削除
          tempUserSelections.delete(interaction.user.id);
    ...
    // セレクトメニューインタラクション（ドロップダウン）
      ...
      // 参加タイプ選択（天元/ルシゼロ/なんでも）
      if (customId.startsWith('join_type_select_')) {
        const recruitmentId = customId.replace('join_type_select_', '');
        const selectedType = interaction.values[0];
        
        // 選択内容を一時保存
        let userSelections = tempUserSelections.get(interaction.user.id) || {};
        userSelections.recruitmentId = recruitmentId;
        userSelections.joinType = selectedType;
        tempUserSelections.set(interaction.user.id, userSelections);
      ...
      // 属性選択
        ...
        let userSelections = tempUserSelections.get(interaction.user.id) || {};
        userSelections.recruitmentId = recruitmentId;
        userSelections.joinType = joinType;
        userSelections.attributes = selectedAttributes;
        tempUserSelections.set(interaction.user.id, userSelections);
        
        await showTimeAvailabilitySelection(interaction, recruitmentId, joinType, selectedAttributes);
      }
      // timemenuの処理（時間選択）
      else if (customId.startsWith('timemenu_')) {
        console.log('本番時間選択を検出: ' + customId);
        ...
        try {
          ...
          let userSelections = tempUserSelections.get(interaction.user.id) || {};
          userSelections.recruitmentId = recruitmentId;
          userSelections.timeAvailability = selectedTime;
          tempUserSelections.set(interaction.user.id, userSelections);
...
// 一時的なユーザー選択データを保持するためのMapを追加
const tempUserSelections = new Map();
